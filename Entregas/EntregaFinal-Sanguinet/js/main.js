...
const obtenerAutosFetch = async () => {
  try {
    if (!JSON.parse(localStorage.getItem("autos"))) {
      //si autos esta vacio (en local storage)
...
export function obtenerAutos() {
  obtenerAutosFetch().then(() => {
    ...
    if (modeloEnSession) {
      ...
      cargarOpciones(0, "inicial"); //cargo todas las opciones (me falto enviarle INICIAL!)
...
function cargarEventoModelos() {
  //carga indica si es "inicial o no", si es inicial significa que es cuando se entra a la pagina
...
function cargarEvento(categoria) {
  ...
  for (let item of input) {
    item.addEventListener("click", () => {
      ...
      if (categoria === "color") {
        //no uso operador avanzado porque es mucho codigo,
...
function cargarOpciones(modeloSel, carga) {
  //inicial es si es la carga inicial
  const autos = JSON.parse(localStorage.getItem("autos"));
